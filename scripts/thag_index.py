#!/usr/bin/env python3
"""
Keep the Theragāthā index and the chain hand-off in step with a content module.

Identical logic to an_index.py/sn_index.py. Kept as its own file for the
same reason thag_build.py is.

Usage:
    python3 scripts/thag_index.py thag_content_01
"""
import importlib
import os
import re
...
INDEX = os.path.join(thag_build.OUT_DIR, "index.html")
...
def main():
    mod = importlib.import_module(sys.argv[1] if len(sys.argv) > 1
                                  else "thag_content_01")
    pages = thag_build.chain(mod)
    ...
    rows = [ROW.format(slug=slug,
                       num=slug.replace("thag-", "Thag ").replace("-", "\u2013"),
    ...
    if tail_href.endswith(".html"):
        path = os.path.join(thag_build.OUT_DIR, tail_href)
        doc = open(path, encoding="utf-8").read()
        last = pages[-1]
        new = ('<a href="%s.html"><span class="label">&larr; Previous</span>%s'
               " &middot; %s</a>"
               % (last["slug"],
                  last["slug"].replace("thag-", "Thag ").replace("-", "&ndash;"),
                  last["nav_title"]))
