#!/usr/bin/env python3
"""
Keep the Khuddakapatha index and the chain hand-off in step with a content module.

Identical logic to an_index.py/sn_index.py -- see those files for the full
rationale. Kept as its own file for the same reason kp_build.py is.

Usage:
    python3 scripts/kp_index.py kp_content_01
"""
import importlib
import os
import re
...
INDEX = os.path.join(kp_build.OUT_DIR, "index.html")
...
def main():
    ...
    rows = [ROW.format(slug=slug,
                       num=slug.replace("kp-", "KP ").replace("-", "\u2013"),
    ...
    if tail_href.endswith(".html"):
        path = os.path.join(kp_build.OUT_DIR, tail_href)
        doc = open(path, encoding="utf-8").read()
        last = pages[-1]
        new = ('<a href="%s.html"><span class="label">&larr; Previous</span>%s'
               " &middot; %s</a>"
               % (last["slug"],
                  last["slug"].replace("kp-", "KP ").replace("-", "&ndash;"),
                  last["nav_title"]))
