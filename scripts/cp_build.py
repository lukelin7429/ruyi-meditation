#!/usr/bin/env python3
"""
Build Cariyapiṭpaka study pages.
...
Usage:
    python3 scripts/cp_build.py cp_content_01
"""
import importlib
import json
import os
...
OUT_DIR = os.path.join(ROOT, "discourses", "cariyapitaka")
...
TRANS = os.path.join(BILARA, "translation/en/sujato/sutta/kn/cp")
...
# --------------------------------------------------------------------------- #
def load_source(rel):
    """rel is like 'cp1' (this collection has no sub-folder per text)."""
...
def build(page, shell):
    ...
    header = (
        ...
        '<a href="./">Cariyapiṭaka</a><span>&rsaquo;</span>%s\n'
        "  </nav>\n\n"
...
def chain(mod):
    ...
    pages = mod.PAGES
    head = getattr(mod, "HEAD", ("./", "Cariyapiṭpaka selections"))
    tail = getattr(mod, "TAIL", ("./", "Cariyapiṭpaka selections"))
    for i, page in enumerate(pages):
        label = lambda p: (p["slug"].replace("cp-", "Cp ").replace("-", "&ndash;")
                           + " &middot; " + p["nav_title"])
...
def main():
    module = sys.argv[1] if len(sys.argv) > 1 else "cp_content_01"
