#!/usr/bin/env python3
"""
Structural + textual acceptance check for Aṅguttara Nikāya study pages.
...
Usage:
    python3 scripts/an_verify.py an_content_01
"""
import html
import importlib
import os
...
def verify(page):
    ...
    # --- text fidelity --------------------------------------------------
    src = an_build.load_source(page["source"])
    expected = [an_build.joined(src, an_build.segments(src, spec))
                for kind, _, spec in
