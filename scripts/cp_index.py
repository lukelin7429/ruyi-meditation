#!/usr/bin/env python3
"""
Keep the Cariyapiṭpaka index and the chain hand-off in step with a content module.

Identical logic to an_index.py/sn_index.py -- see those files for the full
rationale. Kept as its own file for the same reason cp_build.py is.

Usage:
    python3 scripts/cp_index.py cp_content_01
"""
import importlib
import os
import re
...
INDEX = os.path.join(cp_build.OUT_DIR, "index.html")
...
def main():
    ...
    rows = [ROW.format(slug=slug,
                       num=slug.replace("cp-", "Cp ").replace("-", "\u2013"),
    ...
    if tail_href.endswith(".html"):
        path = os.path.join(cp_build.OUT_DIR, tail_href)
        doc = open(path, encoding="utf-8").read()
        last = pages[-1]
        new = ('<a href="%s.html"><span class="label">&larr; Previous</span>%s'
               " &middot; %s</a>"
               % (last["slug"],
                  last["slug"].replace("cp-", "Cp ").replace("-", "&ndash;"),
                  last["nav_title"]))
