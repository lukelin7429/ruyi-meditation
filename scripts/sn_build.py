#!/usr/bin/env python3
"""
Build Saṃyutta Nikāya study pages.

The page shell (head, site header, tab/quiz script) is sliced at runtime out of
discourses/samyutta-nikaya/sn-12.1.html, which is the sole format authority.
...
This is the Saṃyutta Nikāya counterpart of an_build.py, kept as its own file
rather than a shared/parametrized module because the two scripts' constants
(OUT_DIR, REFERENCE, TRANS, the "Saṃyutta Nikāya" breadcrumb text) are wired
in as literals throughout — copying was judged safer than generalizing a
script the Aṅguttara side calls its "sole format authority" and never edits.

Usage:
    python3 scripts/sn_build.py sn_content_01
"""
import importlib
import json
import os
...
OUT_DIR = os.path.join(ROOT, "discourses", "samyutta-nikaya")
REFERENCE = os.path.join(OUT_DIR, "sn-12.1.html")
BILARA = os.path.expanduser("~/Documents/Claude/repos/bilara-data")
TRANS = os.path.join(BILARA, "translation/en/sujato/sutta/sn")
...
# --------------------------------------------------------------------------- #
def load_source(rel):
    """rel is like 'sn1/sn1.1-10'."""
    book = rel.split("/")[0]
    name = rel.split("/")[1]
    path = os.path.join(TRANS, book, name + "_translation-en-sujato.json")
...
def is_structural(key, value):
    ...
    the file heading, and `N.0` is the label of a division within it. The
    segno carries the number, and the guide supplies its own section
    headings. (Identical rule to an_build.py's is_structural — kept in sync
    by hand since bilara-data's segment-key convention is shared across
    nikāyas.)"""
    sub = key.split(":")[1]
...
def segments(src, spec):
    ...
    spec may be a uid ('sn1.1') for the whole discourse, a sub-key range
    within one ('sn12.2:1.1-2.3', inclusive, in document order), or an
...
def joined(src, keys):
    ...
    The Sagāthāvagga is largely verse, and its segments carry a bare `<j>`
    where the line is broken for display. It is markup, not text; left in it
    would print literally. Stripping it here keeps the build and the
    verifier in step, since both read the text through this function."""
...
def build(page, shell):
    ...
    header = (
        ...
        '<a href="./">Saṃyutta Nikāya</a><span>&rsaquo;</span>%s\n'
        "  </nav>\n\n"
...
def chain(mod):
    ...
    """
    pages = mod.PAGES
    head = getattr(mod, "HEAD", ("./", "Saṃyutta Nikāya selections"))
    tail = getattr(mod, "TAIL", ("./", "Saṃyutta Nikāya selections"))
    for i, page in enumerate(pages):
        label = lambda p: (p["slug"].replace("sn-", "SN ").replace("-", "&ndash;")
                           + " &middot; " + p["nav_title"])
