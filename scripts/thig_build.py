...
one, since Therīgāthā is organized into books by verse-count-per-poet, not
one flat number space.

Usage:
    python3 scripts/thig_build.py thig_content_01
"""
import importlib
import json
import os
...
OUT_DIR = os.path.join(ROOT, "discourses", "therigatha")
...
TRANS = os.path.join(BILARA, "translation/en/sujato/sutta/kn/thig")
...
def load_source(rel):
    """rel is like 'thig1.1' -- unlike SN, bilara-data keeps every Therīgāthā
    file flat under kn/thig/, with no per-book subfolder."""
...
def build(page, shell):
    ...
    header = (
        ...
        '<a href="./">Therīgāthā</a><span>&rsaquo;</span>%s\n'
        "  </nav>\n\n"
...
def chain(mod):
    pages = mod.PAGES
    head = getattr(mod, "HEAD", ("./", "Therīgāthā selections"))
    tail = getattr(mod, "TAIL", ("./", "Therīgāthā selections"))
    for i, page in enumerate(pages):
        label = lambda p: (p["slug"].replace("thig-", "Thig ").replace("-", "&ndash;")
                           + " &middot; " + p["nav_title"])
...
def main():
    module = sys.argv[1] if len(sys.argv) > 1 else "thig_content_01"
