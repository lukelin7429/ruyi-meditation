#!/usr/bin/env python3
"""
Structural + textual acceptance check for Khuddakapatha study pages.

Identical checks to an_verify.py/sn_verify.py -- see those files for the
full list. Kept as its own file for the same reason kp_build.py is.

Usage:
    python3 scripts/kp_verify.py kp_content_01
"""
import html
import importlib
import os
...
import kp_build  # noqa: E402

ROOT = kp_build.ROOT
OUT_DIR = kp_build.OUT_DIR
...
def verify(page):
    ...
    src = kp_build.load_source(page["source"])
    expected = [kp_build.joined(src, kp_build.segments(src, spec))
                for kind, _, spec in
    ...
    if block:
        ...
        heads = [(k, v.strip()) for k, v in src.items()
                 if kp_build.is_structural(k, v) and v.strip()]
...
def main():
    ...
    pages = kp_build.chain(importlib.import_module(module))
    for page in pages:
        verify(page)
