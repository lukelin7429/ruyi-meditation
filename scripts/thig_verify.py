#!/usr/bin/env python3
"""
Structural + textual acceptance check for Therīgāthā study pages.

Identical checks to an_verify.py/sn_verify.py. Kept as its own file for the
same reason thig_build.py is.

Usage:
    python3 scripts/thig_verify.py thig_content_01
"""
import html
import importlib
import os
...
import thig_build  # noqa: E402

ROOT = thig_build.ROOT
OUT_DIR = thig_build.OUT_DIR
...
def verify(page):
    ...
    src = thig_build.load_source(page["source"])
    expected = [thig_build.joined(src, thig_build.segments(src, spec))
                for kind, _, spec in
    ...
    if block:
        ...
        heads = [(k, v.strip()) for k, v in src.items()
                 if thig_build.is_structural(k, v) and v.strip()]
...
def main():
    ...
    pages = thig_build.chain(importlib.import_module(module))
    for page in pages:
        verify(page)
