#!/usr/bin/env python3
"""
Keep the Visuddhimagga index and the chain hand-off in step with a content module.

Identical logic to an_index.py/sn_index.py. Kept as its own file for the
same reason vism_build.py is (no CC0 text to pull, so a different template
entirely -- see that file's docstring).

Usage:
    python3 scripts/vism_index.py vism_content_01
"""
import importlib
import os
import re
...
INDEX = os.path.join(vism_build.OUT_DIR, "index.html")
...
def main():
    mod = importlib.import_module(sys.argv[1] if len(sys.argv) > 1
                                  else "vism_content_01")
    pages = vism_build.chain(mod)
    ...
    rows = [ROW.format(slug=slug, num="Ch. %d" % int(re.search(r"\d+", slug).group()),
    ...
    doc = re.sub(r"<dd>\d+ of 23 chapters", "<dd>%d of 23 chapters" % live, doc, count=1)
    ...
    if tail_href.endswith(".html"):
        path = os.path.join(vism_build.OUT_DIR, tail_href)
        if os.path.exists(path):
            doc = open(path, encoding="utf-8").read()
            last = pages[-1]
            new = ('<a href="%s.html"><span class="label">&larr; Previous</span>%s</a>'
                   % (last["slug"], last["nav_title"]))
