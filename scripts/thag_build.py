#!/usr/bin/env python3
"""
Build Theragāthā study pages.

The page shell is sliced at runtime out of discourses/samyutta-nikaya/sn-12.1.html,
which is the sole format authority for the whole site. Discourse text is
never typed by hand: paragraphs name segment keys and the text is pulled
straight from bilara-data (Bhikkhu Sujato, CC0).

Copied from an_build.py/sn_build.py rather than made a shared/parametrized
module -- see sn_build.py's docstring for the rationale. This collection
uses SN's two-level page() signature (book, person) rather than AN's flat
one, since Theragāthā is organized into books by verse-count-per-poet, not
one flat number space.

Usage:
    python3 scripts/thag_build.py thag_content_01
"""
import importlib
import json
import os
...
OUT_DIR = os.path.join(ROOT, "discourses", "theragatha")
...
TRANS = os.path.join(BILARA, "translation/en/sujato/sutta/kn/thag")
...
def load_source(rel):
    """rel is like 'thag1.1' -- unlike SN, bilara-data keeps every Theragāthā
    file flat under kn/thag/, with no per-book subfolder."""
...
def joined(src, keys):
    """This collection is entirely verse; every segment may carry <j>."""
...
def build(page, shell):
    ...
    header = (
        ...
        '<a href="./">Theragāthā</a><span>&rsaquo;</span>%s\n'
        "  </nav>\n\n"
...
def chain(mod):
    pages = mod.PAGES
    head = getattr(mod, "HEAD", ("./", "Theragāthā selections"))
    tail = getattr(mod, "TAIL", ("./", "Theragāthā selections"))
    for i, page in enumerate(pages):
        label = lambda p: (p["slug"].replace("thag-", "Thag ").replace("-", "&ndash;")
                           + " &middot; " + p["nav_title"])
...
def main():
    module = sys.argv[1] if len(sys.argv) > 1 else "thag_content_01"
