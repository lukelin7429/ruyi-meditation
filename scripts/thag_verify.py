#!/usr/bin/env python3
"""
Structural + textual acceptance check for Theragāthā study pages.

Identical checks to an_verify.py/sn_verify.py. Kept as its own file for the
same reason thag_build.py is.

Usage:
    python3 scripts/thag_verify.py thag_content_01
"""
import html
import importlib
import os
...
import thag_build  # noqa: E402

ROOT = thag_build.ROOT
OUT_DIR = thag_build.OUT_DIR
...
def verify(page):
    ...
    src = thag_build.load_source(page["source"])
    expected = [thag_build.joined(src, thag_build.segments(src, spec))
                for kind, _, spec in
    ...
    if block:
        ...
        heads = [(k, v.strip()) for k, v in src.items()
                 if thag_build.is_structural(k, v) and v.strip()]
...
def main():
    ...
    pages = thag_build.chain(importlib.import_module(module))
    for page in pages:
        verify(page)
