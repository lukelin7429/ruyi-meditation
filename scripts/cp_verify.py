#!/usr/bin/env python3
"""
Structural + textual acceptance check for Cariyapiṭpaka study pages.

Identical checks to an_verify.py/sn_verify.py -- see those files for the
full list. Kept as its own file for the same reason cp_build.py is.

Usage:
    python3 scripts/cp_verify.py cp_content_01
"""
import html
import importlib
import os
...
import cp_build  # noqa: E402

ROOT = cp_build.ROOT
OUT_DIR = cp_build.OUT_DIR
...
def verify(page):
    ...
    src = cp_build.load_source(page["source"])
    expected = [cp_build.joined(src, cp_build.segments(src, spec))
                for kind, _, spec in
    ...
    if block:
        ...
        heads = [(k, v.strip()) for k, v in src.items()
                 if cp_build.is_structural(k, v) and v.strip()]
...
def main():
    ...
    pages = cp_build.chain(importlib.import_module(module))
    for page in pages:
        verify(page)
