#!/usr/bin/env python3
"""
Build Jātaka (selected verses) study pages.
...
Usage:
    python3 scripts/ja_build.py ja_content_01
"""
import importlib
import json
import os
...
OUT_DIR = os.path.join(ROOT, "discourses", "jataka")
...
TRANS = os.path.join(BILARA, "translation/en/sujato/sutta/kn/ja")
...
# --------------------------------------------------------------------------- #
def load_source(rel):
    """rel is like 'ja1' (this collection has no sub-folder per text)."""
...
def build(page, shell):
    ...
    header = (
        ...
        '<a href="./">Jātaka</a><span>&rsaquo;</span>%s\n'
        "  </nav>\n\n"
...
def chain(mod):
    ...
    pages = mod.PAGES
    head = getattr(mod, "HEAD", ("./", "Jātaka (selected verses) selections"))
    tail = getattr(mod, "TAIL", ("./", "Jātaka (selected verses) selections"))
    for i, page in enumerate(pages):
        label = lambda p: (p["slug"].replace("ja-", "Ja ").replace("-", "&ndash;")
                           + " &middot; " + p["nav_title"])
