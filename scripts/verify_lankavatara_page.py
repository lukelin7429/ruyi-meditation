#!/usr/bin/env python3
"""Verify a lankavatara-sutra page against the CBETA base text.

Usage: python3 verify_lankavatara_page.py <page.html> <start_line> <end_line>
...
BASE = "/Users/hayashikisshou/Library/Mobile Documents/iCloud~md~obsidian/Documents/第二個大腦/知識庫/佛法/大正藏/09_經集部/T0670_楞伽阿跋多羅寶經.md"
...
def extract_top_level_blocks(html, wanted_token):
    """Return contents of every top-level div whose class token set contains wanted_token.
    'Top-level' means not nested inside another div already matched as target
    (prevents double-counting text if a target div were ever nested in another)."""
    blocks = []
    depth = 0
    target_stack = []  # depths at which a target div is open
