#!/usr/bin/env python3
"""
Build Khuddakapatha study pages.
...
for the whole site. Everything between the shell halves is assembled here
from a per-page dict.
...
Copied from an_build.py/sn_build.py rather than made a shared/parametrized
module, for the same reason those two are kept separate from each other (see
sn_build.py's docstring).

Usage:
    python3 scripts/kp_build.py kp_content_01
"""
import importlib
import json
import os
...
OUT_DIR = os.path.join(ROOT, "discourses", "khuddakapatha")
...
TRANS = os.path.join(BILARA, "translation/en/sujato/sutta/kn/kp")
...
# --------------------------------------------------------------------------- #
def load_source(rel):
    """rel is like 'kp1' (this collection has no sub-folder per text)."""
    path = os.path.join(TRANS, rel + "_translation-en-sujato.json")
    with open(path, encoding="utf-8") as fh:
        return json.load(fh)


def is_structural(key, value):
    """Headings are navigation furniture, not discourse text. Identical rule
    to an_build.py's is_structural -- see that file for the full rationale."""
    sub = key.split(":")[1]
    return sub.startswith("0.") or sub.endswith(".0")


def segments(src, spec):
    """Which source segments a text paragraph is made of. Identical logic to
    an_build.py/sn_build.py's segments() -- see those files for the spec."""
...
def joined(src, keys):
    ...
    this collection is entirely or mostly verse, so this matters throughout,
    not just occasionally."""
    return "".join(src[k] for k in keys).replace("<j>", "").strip()


# --------------------------------------------------------------------------- #
# blocks (identical to an_build.py/sn_build.py)
# --------------------------------------------------------------------------- #
...
def build(page, shell):
    ...
    header = (
        ...
        '<a href="./">Khuddakapatha</a><span>&rsaquo;</span>%s\n'
        "  </nav>\n\n"
...
def chain(mod):
    """Fill in prev/next from the module's page order. Identical logic to
    an_build.py/sn_build.py's chain()."""
    pages = mod.PAGES
    head = getattr(mod, "HEAD", ("./", "Khuddakapatha selections"))
    tail = getattr(mod, "TAIL", ("./", "Khuddakapatha selections"))
    for i, page in enumerate(pages):
        label = lambda p: (p["slug"].replace("kp-", "KP ").replace("-", "&ndash;")
                           + " &middot; " + p["nav_title"])
...
def main():
    module = sys.argv[1] if len(sys.argv) > 1 else "kp_content_01"
